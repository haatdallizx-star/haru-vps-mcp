...
public final class HealthKitManager {
    ...
    var readTypes: Set<HKObjectType> {
        Set(HealthMetric.allCases.compactMap { $0.sampleType })
    }

    public static func initialSyncStart(now: Date = Date()) -> Date {
    ...
    func anchoredQuery(
        ...
        startDate: Date?,
        handler: @escaping ([HKSample], [HKDeletedObject], HKQueryAnchor?, Error?) -> Void
        ...
        return HKAnchoredObjectQuery(type: type, predicate: predicate, anchor: anchor, limit: HKObjectQueryNoLimit) {
            ...
            handler(samples ?? [], deletions ?? [], newAnchor, error)
        }
    }
